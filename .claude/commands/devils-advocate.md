...
Review ONLY the diff between the current branch and the default branch.

Detect the default branch automatically — try `main` first, fall back to `master`:

```
!`git diff $(git rev-parse --verify main >/dev/null 2>&1 && echo main || echo master)...HEAD 2>/dev/null`
```

If the diff is empty or the command fails, say so clearly:
- Empty diff: "No changes detected against the default branch. Nothing to review."
- Command failure: "Could not run git diff — ensure you are in the project directory with at least one commit on a branch that diverges from main/master."

Do not proceed with a review if there is no diff.
